...
@pytest.mark.parametrize("mode,userinfo,existing_entries", [
...
@pytest.mark.parametrize("existing_entries,username", [
    ([], None),
...
@pytest.mark.parametrize("userinfo", [USERINFO])
@pytest.mark.parametrize("mode", ["read_only", "pre_created", "full_access"])
@pytest.mark.parametrize("existing_entries,groupname", [
    ([], None),
...
@pytest.mark.parametrize("userinfo", [USERINFO])
@pytest.mark.parametrize("mode", ["read_only"])
@pytest.mark.parametrize("existing_entries,groups", [
    ([], []),
    ([LDAP_USER_ENTRY], []),
    ([LDAP_USER_ENTRY, LDAP_PRECREATED_TESTGROUP_ENTRY], []),
    ([LDAP_USER_ENTRY, LDAP_TESTGROUP_ENTRY], ["testgroup"]),
    ([LDAP_USER_ENTRY, LDAP_TESTGROUP_ENTRY, LDAP_TESTGROUP2_ENTRY], ["testgroup", "testgroup2"]),
])
...
@pytest.mark.parametrize("userinfo,existing_entries", [
...
def test_mod_user_ignore(ldap_mod_entry):
    ...
    ldap_mod_entry["user"].mod(
        ldap_mod_entry["supplementary_groups"],
        ldap_mod_entry["removal_groups"]
    )
